...
def tracking_page(request):
    ...
    tracked_qs = (TrackedItem.objects
...
def my_tracking(request):
    tracked = (TrackedItem.objects
        .filter(user=request.user)
        .select_related('item')
        .prefetch_related('marketplaces'))
    return render(request, 'tracking/index.html', {'tracked': tracked})


@login_required(login_url=login_url)
def remove_tracking(request, item_id):
    if request.method == 'POST':
        TrackedItem.objects.filter(user=request.user, item_id=item_id).delete()
    return redirect('tracking:tracking')